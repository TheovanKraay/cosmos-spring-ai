...
SEMVER_REGEX='^[0-9]+\.[0-9]+\.[0-9]+(-beta\.[0-9]+|-M[0-9]+|-RC[0-9]+)?$'
...
if [[ "$VERSION" =~ $SEMVER_REGEX ]]; then
    pass "Version '$VERSION' follows X.Y.Z[-beta.N | -MN | -RCN]"
else
    fail "Version '$VERSION' does not match X.Y.Z[-beta.N | -MN | -RCN]"
fi
    ...
    prepare)
        ...
        if [[ -n "$AUTOCONF_PROP" ]]; then
            ...
            if [[ -z "$CORE_DEP" ]]; then
                ...
                fail "<$AUTOCONF_PROP> ($CORE_DEP) is not a valid release version (X.Y.Z[-beta.N | -MN | -RCN])"
            else
